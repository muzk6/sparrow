...
class AppXdebug
{
    ...
    public function auto()
    {
        ...
        // 从 cgi 开启
        if (app(AppWhitelist::class)->isSafeIp()) {
        ...
        if (!$traceStart && file_exists($traceConfFile)) {
            ...
            if ($traceConf['expire'] > time() // 检查过期
                ...
                && (!$traceConf['user_id'] || (app(AppAuth::class)->isLogin() && $traceConf['user_id'] == app(AppAuth::class)->getUserId())) // 有指定用户时，检查特定用户
            ) {
                $traceStart = true;
